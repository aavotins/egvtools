...
#' 4) **Align**: `terra::project(raw_crop, template, method)` → `terra::mask(..., template)`.  
...
#'
#' **IDW smoothing**
#' Aggregate → points → IDW back to template. Controls:
...
downscale2egv <- function(
  ...
  if (isTRUE(smooth)) {
    ...
    if (is_cat_now && !isTRUE(smooth_force)) {
      ...
      if (terra::is.lonlat(template_r)) warning("Template is lon/lat; IDW radius will be in degrees after km→deg conversion.")
