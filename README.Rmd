...
coordinate reference system, exact pixel matching, etc.:
 ...
 - `generalized_radius_function()` - multi-scale zonal statistics (dense/sparse) 
 for user defined zones, following naming convention of `tiled_buffers()`;
...
resolution. While `radius_function()` and `generalized_radius_function()` creates 
output matching EGV template with 
cell values representing aggregated information from larger scales.
